 ...
 * Countdown shooting, slide the slider to set the duration of the countdown shooting
 */
...
public class CountDownDurationAdjustView extends RelativeLayout {
    ...
    /*
     * mHasFinishedDuraViewWidth 值为0，表示未进行过倒计时拍摄；不为0，则已进行一次或者多次倒计时拍摄
     * The mHasFinishedDuraViewWidth value is 0, which means that no countdown shooting has been performed; if it is not 0, one or more countdown shooting has been performed
     * */
    ...
    private void setNewDurationChangeCallback(boolean isDragEnd) {
        if (mNewDurationChangeListener != null) {
            if (mNewCaptureDuration < MIN_DURATION) {
                mNewCaptureDuration = MIN_DURATION;
            }
