...
public class GrallyAdapter extends RecyclerView.Adapter<GrallyAdapter.ViewHolder> implements ItemTouchListener {
    ...
    private void setBindViewHolder(final ViewHolder holder, int position) {
        ...
        boolean isLoadHttpUrl = false;
        if (!TextUtils.isEmpty(filePath) && filePath.startsWith("https://") && filePath.endsWith(".mp4")) {
            filePath = mClipInfoArray.get(position).getCoverPath();
        ...
        if (!isLoadHttpUrl) {
            ...
            WebpDrawableTransformation webpDrawableTransformation = new WebpDrawableTransformation(fitCenter);
            Glide.with(mContext)
                    .load(filePath)
                    .apply(options)
                    .frame(mClipInfoArray.get(position).getTrimIn())
            ...
            Glide.with(mContext)
                    .asBitmap()
                    .override(200, 200)
                    .load(TextUtils.isEmpty(filePath) ? "" : Uri.parse(filePath))
                    .apply(options.frame(mClipInfoArray.get(position).getTrimIn()))
                    .into(holder.mImageView);
        }
