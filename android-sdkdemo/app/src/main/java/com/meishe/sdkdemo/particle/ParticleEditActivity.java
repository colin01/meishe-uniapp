...
public class ParticleEditActivity extends BaseActivity {
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_TYPE_PARTICLE_FX:
                    ...
                    long time_total = mTimeline.getDuration();
                    long time_cur = mSeekBegin;
                    long time_each_frame = 1000 * 1000 / 25; // 每帧时长 Length per frame
                ...
                switch (motionEvent.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        if (mCurrentParticleFxId != null && !mCurrentParticleFxId.isEmpty() && mStreamingContext != null) {
                            ...
                            if (null != mCurrentVideoFx) {
                                mCurrentVideoFx.setFloatVal("Tail Fading Duration", 0.5); // 拖尾效果的时长（s） Length of trailing effect (s)
                    ...
                    case MotionEvent.ACTION_MOVE:
                        long cur_us_timeline = mStreamingContext.getTimelineCurrentPosition(mTimeline);
                        long cur_ms_system = TimeFormatUtil.getCurrentTimeMS();
                        long time_each_seek = (cur_ms_system - mMoveSystemBegin) * 1000;
    ...
    private void setViewClickable(boolean isClick) {
        mBackBtn.setClickable(isClick);
        mPlayBtn.setClickable(isClick);
        mSaveBtn.setClickable(isClick);
    }
    ...
    private void updateFilterDataList() {
        ...
        if (bundleFilterData.size() > 0) {
            //bundle data 放置到列表的前面
            //The bundle data is placed first in the list
            mParticleFxList.addAll(0, bundleFilterData);
        }
    ...
    private void initTimeline() {
        ...
        int makeRatio = POINT9V16;
        if (mVideoWidth > mVideoHeight) {
            makeRatio = POINT16V9;
        }
        NvsVideoResolution videoEditRes = Util.getVideoEditResolution(makeRatio);
        Log.d(TAG, "videoEditResolution.imageWidth==" + videoEditRes.imageWidth + " videoEditResolution.imageHeight==" + videoEditRes.imageHeight);
        if (videoEditRes.imageWidth * videoEditRes.imageHeight > 2160 * 3840) {
            ToastUtil.showToast(this, getResources().getString(R.string.timeline_size_to_large));
            return;
        }
        mTimeLineWidth = videoEditRes.imageWidth;
        mTimeLineHeight = videoEditRes.imageHeight;
        ...
        for (int i = 0; i < mPath.size(); i++) {
            NvsVideoClip clip = mVideoTrack.appendClip(mPath.get(i));
