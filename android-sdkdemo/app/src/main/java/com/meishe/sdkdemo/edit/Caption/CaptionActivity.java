...
public class CaptionActivity extends BaseActivity {
    ...
    protected void initViews() {
        ...
        llSeekContainer = findViewById(R.id.ll_seek_1);
        seekBarOpacityWidthValue = findViewById(R.id.seekBarOpacityWidthValue);
        alphaSeek.setMax(100);
        seekBarOpacityWidthValue.setText(String.valueOf(100));
...
//        }
        llSeekContainer.setVisibility(mCurCaption == null ? View.GONE : View.VISIBLE);
    }
        ...
        alphaSeek.setOnSeekBarChangeListener(new OnSeekBarChangeListenerAbs() {
            ...
            public void onProgressChanged(SeekBar seekBar, int i, boolean b) {
                ...
                if (mCurCaption != null) {
                    ...
                    seekBarOpacityWidthValue.setText(String.valueOf(i));
            ...
            mVideoFragment.setAssetEditListener(new VideoFragment.AssetEditListener() {
                ...
                public void onAssetDelete() {
                    ...
                    if (mCurCaption != null) {
                        ...
                        seekBarOpacityWidthValue.setText(String.valueOf((int) (mCurCaption.getOpacity() * alphaSeek.getMax())));
                    }
                }
                ...
                public void onAssetSelected(PointF curPoint) {
                    if (mAddKeyFrame) {
                        //关键帧模式 不切换选中的字幕
                        //Keyframe mode does not switch selected subtitles
                        return;
                    }
                    /*
...
//                    mIsInnerDrawRect = false;
    ...
    private void updateCaptionRelativeView(long timeStamp) {
        ...
        if ((mCurCaption != null)
            ...
            seekBarOpacityWidthValue.setText(String.valueOf((int) ((floatValAtTime) * alphaSeek.getMax())));
        ...
        if (mCurCaption != null) {
            if (timeStamp > mCurCaption.getOutPoint() + 1000 || timeStamp < mCurCaption.getInPoint() - 1000) {
                ...
                //Hide box outside the current subtitle time Disable all keys button
                mVideoFragment.setDrawRectVisible(View.GONE);
                displayEnable(false);
            } else {
                // seek到关键帧的位置的时候，选中一下当前的关键帧，使以后所有的操作，在当前帧下操作.
                //When you seek to the position of the key frame, select the current key frame so that all subsequent operations can be performed at the current frame
    ...
    private void updateKeyFrameView(long timeStamp, Map<Long, KeyFrameInfo> keyFrameMap) {
        if (keyFrameMap == null || keyFrameMap.isEmpty()) {
            ...
            for (Map.Entry<Long, KeyFrameInfo> entry : entries) {
                ...
                if (key > timeStamp) {
                    // 找到距离当前位置 向后最近的一个时间点
                    //Find a time point closest to the current position
                    nextKeyFrame = key;
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.iv_zoom_in) {
            ...
            Bundle bundle;//                new InputDialog(this, R.style.dialog, new InputDialog.OnCloseListener() {
//                    @Override
//                    public void onClick(Dialog dialog, boolean ok) {
//                        if (ok) {
//                            InputDialog d = (InputDialog) dialog;
...
//                }).show();
     ...
     * Whether the new key frame retrieval has control point information, and if it does,
     * it is applied to the application layer
     *
     * @param keyFrameInfo
        ...
        switch (requestCode) {
            case REQUESTCAPTIONSTYLE:
                ...
                if (!isSelectCurCaption) {
                    ...
                    int curZVal = data.getIntExtra("isSelectCurCaptionZValue", 0);
    ...
    private void selectCaption() {
        ...
        if (mCurCaption != null) {
            ...
            if (captionInfo != null) {
                ...
                mVideoFragment.setScale(scaleFactorX);
            }
        }
    }
