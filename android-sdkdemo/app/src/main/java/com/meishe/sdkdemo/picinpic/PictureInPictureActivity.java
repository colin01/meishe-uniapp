...
//        });
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.replaceImage) {
            ...
            pipBundle.putInt("picInPicVideoIndex", mClipVideoIndex);
        ...
        } else if (id == R.id.upVideoVolumeButton) {
            mUpClipIsMute = !mUpClipIsMute;
            if (mUpClipIsMute) {
                mVideoTrackFir.setVolumeGain(muteVolumeValue, muteVolumeValue);
                mUpVideoVolumeButton.setImageResource(R.mipmap.icon_pip_mute);
            } else {
                mVideoTrackFir.setVolumeGain(defaultVolumeValue, defaultVolumeValue);
                mUpVideoVolumeButton.setImageResource(R.mipmap.icon_pip_volume);
            }
        } else if (id == R.id.downVideoVolumeButton) {
            mDownClipIsMute = !mDownClipIsMute;
            if (mDownClipIsMute) {
                mVideoTrackSec.setVolumeGain(muteVolumeValue, muteVolumeValue);
                mDownVideoVolumeButton.setImageResource(R.mipmap.icon_pip_mute);
            } else {
                mVideoTrackSec.setVolumeGain(defaultVolumeValue, defaultVolumeValue);
                mDownVideoVolumeButton.setImageResource(R.mipmap.icon_pip_volume);
            }
        } else if (id == R.id.templateLayout) {
            mTemplateListLayout.setVisibility(View.VISIBLE);
        } else if (id == R.id.previewLayout) {
            if (Util.isFastClick()) {
                return;
            }
    ...
    private void resetVari() {
        mFirScaleValue = mSecScaleValue = 1.0D;      //缩放 scale
        mFirRotateAngle = mSecRotateAngle = 0;       //旋转 rotation
        mUpClipIsMute = mDownClipIsMute = false;     //
        mUpTransX = mUpTransY = 0;                  //x轴移动 X-axis movement
        mDownTransX = mDownTransY = 0;              //y轴移动 Y-axis movement
    }
    ...
    private void readPicInPicJsonFileInfo(String jsonFilePath, boolean isBundle) {
        try {
            ...
            ArrayList<PicInPicTemplateAsset.PicInPicJsonFileInfoList> jsonFileInfoLists = fromJson(strbuilder.toString(), new TypeToken<List<PicInPicTemplateAsset.PicInPicJsonFileInfoList>>() {
            }.getType());
            ...
            e.printStackTrace();
            Log.e(TAG, "readPicInPicJsonFileInfo: " + e);
        }
    }
    ...
    private ArrayList<String> getPackageIdList(String fileDir, PicInPicTemplateAsset.PicInPicJsonFileInfoList jsonFileInfo) {
        ...
        String packageId = installAssetPackage(videoFxPackagePath);
        if (!packageId.isEmpty()) {
            videoFxPackageIDList.add(packageId);
        }
        videoFxPackagePath = fileDir + jsonFileInfo.getPipPackageName2();
        packageId = installAssetPackage(videoFxPackagePath);
        if (!packageId.isEmpty()) {
            videoFxPackageIDList.add(packageId);
        }
    ...
    private String installAssetPackage(String videoFxPackagePath) {
        return PackageManagerUtil.installAssetPackage(videoFxPackagePath, null, NvsAssetPackageManager.ASSET_PACKAGE_TYPE_VIDEOFX);
    }
    ...
    private void applyVideoFxTemplate(int index) {
        ...
        for (int firstIndex = 0; firstIndex < firstClipCount; ++firstIndex) {
            ...
            if (mPiPTemplateAssetsArray != null && index >= 0 && index < mPiPTemplateAssetsArray.size()) {
    ...
    private void setLiveWindowRatio(int ratio) {
        ...
        int navBarHeight = ScreenUtils.getNavigationBarHeight(this);
